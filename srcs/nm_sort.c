...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/05/12 17:29:54 by angavrel          #+#    #+#             */
/*   Updated: 2018/05/13 22:06:44 by angavrel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	alphabetically(t_symbol *best_match, t_symbol *other, bool reverse)
{
	int			diff;
	...
	diff = ft_strncmp(best_match->string, other->string, \
		...
		best_match->str_max_size : other->str_max_size);
	if (!diff)
		return (reverse == (best_match->offset < other->offset));
	return (reverse == (0 > diff));
}

static bool	numerically(t_symbol *best_match, t_symbol *other, bool reverse)
{
	if (best_match->offset == other->offset)
		return (alphabetically(best_match, other, reverse));
	return (reverse == (best_match->offset < other->offset));
}

void		nm_selection_sort(t_sym_sort *sorted_symbols, uint8_t sort_type)
{
	static bool (*compare[2])(t_symbol *, t_symbol *, bool) = {
	...
	while (i < sorted_symbols->nsyms_sort)
	{
		...
		while (j < sorted_symbols->nsyms_sort)
		{
			if (compare[sort_type >> 1](symbols_ptr[best_match], \
					symbols_ptr[j], (sort_type & 1)))
				best_match = j;
			j++;
		}
		swap = symbols_ptr[best_match];
